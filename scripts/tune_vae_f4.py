...
def objective(trial, args, device):
    ...
    channel_mults_ = trial.suggest_categorical(
        "channel_mults", ['124', '123', '134', '122', '112','113']
    )
    channel_mults = [int(c) for c in channel_mults_]
    num_channels = [start_channels * mult for mult in channel_mults]
    attention_level_2 = trial.suggest_categorical(
        "attention_level_2", [True, False]
    )
    attention_leve_3 = trial.suggest_categorical(
        "attention_level_3", [True, False]
    )
    attention_levels = [False, attention_level_2, attention_leve_3]
    # Training
    ...
    for epoch in range(args.trial_epochs):
        ...
        for i, (batch, _, _) in progress_bar:
            ...
            with torch.amp.autocast(device_str, enabled=True):
                ...
                kl_loss = 0.5 * torch.sum(z_mu.pow(2) + z_sigma.pow(2) - torch.log(z_sigma.pow(2) + 1e-7) - 1, dim=[1, 2, 3])
...
def save_best_config(best_params, template_config, output_path):
    ...
    norm_num_groups = 32 if min(num_channels) >= 32 else min(num_channels)
    attention_level_2 = best_params["attention_level_2"]
    attention_level_3 = best_params["attention_level_3"]
    attention_levels = [False, attention_level_2, attention_level_3]
    ...
    autoencoder_def["attention_levels"] = attention_levels
    ...
    autoencoder_train['lr'] = best_params["lr"]
...
if __name__ == "__main__":
    ...
    study_name = f"vae_tuning_{args.n_trials}_trials_f4"
    ...
    if study.best_trial:
        ...
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
        tuned_config_path = os.path.join(project_root, "configs", "tune_f4.json")
